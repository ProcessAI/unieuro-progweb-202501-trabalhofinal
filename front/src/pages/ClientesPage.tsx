...
async function toggleStatusSede(sedeToToggle: Sede) { // <-- Altera para receber o objeto 'Sede'
  ...
  try {
    ...
    const sedeResponse = await fetch(`http://localhost:8080/api/sede/${sedeToToggle.idsede}`, {
      method: 'PUT',
      headers: {
        'Content-Type': 'application/json',
      },
    ...
    if (sedeToToggle.endereco && sedeToToggle.endereco.id) {
      ...
      const enderecoResponse = await fetch(`http://localhost:8080/api/endereco/${enderecoId}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
